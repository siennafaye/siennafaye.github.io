(function() {
    ...
    myForm.addEventListener('submit', function(event){
        ...
        if (firstName && adjective && desire && feeling) {
            myText = `Hello, <a>${firstName}</a>!<br>
            
            <br>The cards have something very <a>${adjective}</a> in store for you. I see you desire <a>${desire}</a>. I pulled the <a>${cardMinor}</a>—you’re in luck! <a>${desire}</a> is in your future. New beginnings with <a>${desire}</a> are coming soon. So you’ve been feeling <a>${feeling}</a>? It could be because of the <a>${cardMajor}</a> card. This is the card of <a>${cardMajorMeaning}</a>.<br> 
            
            <br>Remember, this phase is temporary, so you can either enjoy it while it lasts or ride it out until it’s over! I hope you enjoyed your reading, see you next time.<br>
